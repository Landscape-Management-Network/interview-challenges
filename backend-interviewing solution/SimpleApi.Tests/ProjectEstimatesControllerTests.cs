...
public sealed class ProjectEstimatesControllerTests : IDisposable
{
    ...
    [Fact]
    public async Task CreateProjectEstimate_OnDemand_WithinTravelThreshold_CalculatesBaseRateOnly()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ProjectName = "On-Demand Consultation", 
            ClientName = "Frank Miller", 
            ClientEmail = "frank@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 10m // Within 15 miles, no travel fee
        };
        ...
        var createdResult = Assert.IsType<CreatedAtActionResult>(result.Result);
        var createdEstimate = Assert.IsType<ProjectEstimate>(createdResult.Value);
        Assert.Equal(150m, createdEstimate.TotalEstimatedCost); // Base rate only
    }

    [Fact]
    public async Task CreateProjectEstimate_OnDemand_ExactlyAtTravelThreshold_CalculatesBaseRateOnly()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ...
            ClientEmail = "grace@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 15m // Exactly at threshold, no travel fee
        };
    ...
    [Fact]
    public async Task CreateProjectEstimate_OnDemand_BeyondTravelThreshold_CalculatesWithTravelFee()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ProjectName = "On-Demand Remote Service", 
            ClientName = "Henry Lee", 
            ClientEmail = "henry@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 20m // Beyond 15 miles, travel fee applies
        };
        ...
        var createdResult = Assert.IsType<CreatedAtActionResult>(result.Result);
        var createdEstimate = Assert.IsType<ProjectEstimate>(createdResult.Value);
        Assert.Equal(175m, createdEstimate.TotalEstimatedCost); // 150 + 25 travel fee
    }

    [Fact]
    public async Task CreateProjectEstimate_OnDemand_JustBeyondThreshold_CalculatesWithTravelFee()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ProjectName = "On-Demand Emergency", 
            ClientName = "Iris Chen", 
            ClientEmail = "iris@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 15.01m // Just over threshold
        };
    ...
    [Fact]
    public async Task CreateProjectEstimate_OnDemand_ZeroDistance_CalculatesBaseRateOnly()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ProjectName = "On-Site On-Demand", 
            ClientName = "Jack White", 
            ClientEmail = "jack@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 0m // At base location
        };
    ...
    [Fact]
    public async Task CreateProjectEstimate_OnDemand_VeryFarDistance_CalculatesWithTravelFee()
    {
        // Arrange
        var estimate = new ProjectEstimate 
        { 
            ProjectName = "On-Demand Far Away", 
            ClientName = "Kelly Brown", 
            ClientEmail = "kelly@test.com",
            EstimateKind = "OnDemand",
            DistanceFromBase = 100m // Very far from base
        };
        ...
        var createdResult = Assert.IsType<CreatedAtActionResult>(result.Result);
        var createdEstimate = Assert.IsType<ProjectEstimate>(createdResult.Value);
        Assert.Equal(175m, createdEstimate.TotalEstimatedCost); // 150 + 25 travel fee (flat fee, not distance-based)
    }
